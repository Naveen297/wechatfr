  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50 p-4">
      <div className="max-w-7xl mx-auto">
        {/* Top Navigation Bar */}
        <div className="bg-white rounded-xl shadow-lg p-4 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          {/* Left Side */}
          <div className="flex items-center gap-4">
            <Heart className="w-8 h-8 text-pink-500" fill="currentColor" />
            <h1 className="text-2xl font-bold gradient-text">LoveConnect</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="text-sm text-gray-600">
              ...
              <span className="font-mono">
                {roomId.length > 20 ? `${roomId.substring(0, 20)}...` : roomId}
              </span>
            </div>
            <button
              onClick={copyRoomLink}
              className="bg-gradient-to-r from-pink-500 to-purple-600 text-white px-4 py-2 rounded-lg font-semibold hover:shadow-lg transition-premium hover-lift flex items-center gap-2 text-sm"
            >
              {copied ? (
                <>
                ...
                <>
                  ...
                  {isEnglish ? 'Copy Link' : 'Копіювати посилання'}
                </>
              )}
            </button>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 w-full h-[calc(100vh-120px)] min-h-[600px]">
          {/* Chat Section - Left Side (20%) */}
          <div className="w-full lg:w-[20%] h-full">
            <Chat roomId={roomId} isEnglish={isEnglish} isConnected={isConnected} />
          </div>

          {/* Video Section - Right Side (80%) */}
          <div className="flex-1 lg:w-[80%] h-full rounded-2xl overflow-hidden shadow-2xl">
            <VideoChat
              roomId={roomId}
              isEnglish={isEnglish}
